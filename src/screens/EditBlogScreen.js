...
export default function EditBlogScreen({ route, navigation }) {
  ...
  useEffect(() => {
    ...
    const currentTags = tags.trim() ? tags.split(',').map(tag => tag.trim()).filter(tag => tag) : [];
  ...
  }, [title, summary, content, tags, imageUrl, blog]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'Failed to update blog post. Please try again.');
    } finally {
      setLoading(false);
    }
  };
